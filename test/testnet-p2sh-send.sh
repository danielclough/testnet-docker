...
redeemScript=`/opt/btcdeb/btcc ${futureBlock} OP_CHECKLOCKTIMEVERIFY OP_DROP  ${pubkey} OP_CHECKSIG 2> /dev/null`
...
p2shAddress=${p2shAddress}" > p2sh.txt
...
rawtxhex=`docker exec blackcoin-more-api_blackmore-testnet_1 blackmore-cli -testnet createrawtransaction '''[ { "txid": "'${utxo_txid}'", "vout": '${utxo_vout}' } ]''' '''{ "'${recipient}'": "'${amountToSend}'", "'${changeAddress}'": "'${change}'"}''' ${futureBlock}`
# echo -e "
# rawtxhex=${rawtxhex}
# "
